...
func (this *Admin) Add(account string, password string) (Admin, error) {
	var varUser Admin
	varUser.Account = account

	has := md5.Sum([]byte(password))
	md5_password := fmt.Sprintf("%x", has) //将[]byte转成16进制
	varUser.Password = md5_password
	print("Creando user ")
	/*if !libs.DB.Where("account = ? ", varUser.Account).First(&Admin{}).RecordNotFound() {
		return UserModel{}, errors.New("Ya existe un item con el nombre " + varUser.Account)
	}*/
